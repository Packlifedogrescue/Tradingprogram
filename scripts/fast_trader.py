...
Exit rules:
  stop loss (22%), time-weighted trailing stop + breakeven lock,
...
async def check_position(session: aiohttp.ClientSession,
    ...
    # Time-weighted tiered trailing stop + breakeven lock
    is_0dte = pos.get("dte", 99) <= 1
    trail_start = 0.15 if is_0dte else cfg.trail_start_pct
    if peak_pct >= trail_start:
        if is_0dte:
            hour = _now_et().hour + _now_et().minute / 60.0
            if hour < 12.0:            # before noon: wide -- let SPX run
                if   peak_pct >= 3.00: trail = 0.05
                elif peak_pct >= 1.00: trail = 0.08
                elif peak_pct >= 0.50: trail = 0.12
                else:                  trail = 0.15
            elif hour < 14.0:          # noon-2 PM: medium
                if   peak_pct >= 3.00: trail = 0.04
                elif peak_pct >= 1.00: trail = 0.07
                elif peak_pct >= 0.50: trail = 0.10
                else:                  trail = 0.13
            else:                      # after 2 PM: tight -- theta burning
                if   peak_pct >= 3.00: trail = 0.03
                elif peak_pct >= 1.00: trail = 0.05
                elif peak_pct >= 0.50: trail = 0.08
                else:                  trail = 0.10
        else:
            if peak_pct >= 3.00:   trail = 0.10
            elif peak_pct >= 1.00: trail = 0.15
            else:                  trail = cfg.trail_stop_pct
        # Breakeven lock: trail floor can never drop below entry price
        effective_floor = max(new_peak * (1 - trail), entry)
